...
def P_to_Pascal(data, input) :
    ...
    if data['press_unit'] == "psig" :
        return (float(data[input1])+14.7)* 6894.75728 , (float(data[input2])+14.7)* 6894.75728
...
def D_to_meter(data, d, pipe_unit) :
    if pipe_unit == "in" :
        return (float(data['inletD']) - float(data['wallThick'])*2 )*0.0254 , (float(data['outletD']) - float(data['wallThick'])*2 )*0.0254 , d*0.0254
      
    if pipe_unit == "mm" :
        return (float(data['inletD']) - float(data['wallThick'])*2 )*0.001 , (float(data['outletD']) - float(data['wallThick'])*2 )*0.001 , d*0.001
    if pipe_unit == "m" :
        return (float(data['inletD']) - float(data['wallThick'])*2 ) , (float(data['outletD']) - float(data['wallThick'])*2 ) , d
...
def valve_sizing():
    ...
    if valid_data:
        ...
        # d=25.4/1000
        print('D1,D2,d for sizing calculations in mm: ', D1*1000, D2*1000,d*1000)
        print('Cv to K: ', Cv_to_K(Cv=6, D=12.7/1000))
        
       
        #Sizing calculation considering D, Valve (d) and FL - Fd = 1 FLow to Close
        sizing = size_control_valve_l(rho, Psat, Pc, mu, P1, P2, Q=F, D1=D1, D2=D2,
                         d=d, FL=array_FL[-1], Fd=1, full_output=True)
        ...
        graph.scatter(PTravel[0], P_Cv[0], color='red', marker='+', linewidth=5)
        graph.scatter(PTravel[0], FL_Cv[0]*100, color='red', marker='+', linewidth=5)
    ...
    if (min_exist)  :
        ...
        graph.scatter(PTravel[1], P_Cv[1], color='red', marker='+', linewidth=5)
        graph.scatter(PTravel[1], FL_Cv[1]*100, color='red', marker='+', linewidth=5)
    ...
    if (max_exist) :
        ...
        graph.scatter(PTravel[2], P_Cv[2], color='red', marker='+', linewidth=5)
        graph.scatter(PTravel[2], FL_Cv[2]*100, color='red', marker='+', linewidth=5)
...
# Diameter in meters (Pipe ID) Pressure in Pascal , Flow in m3-seg and T in Kelvin for Cv computation
    ...
    if valid_data:
        ...
        sizing = size_control_valve_l(rho, Psat, Pc, mu, P1, P2, F, FL=0.9, Fd=1, full_output=True)
        print('All data Output: ', sizing)
        Cav_index = round(cavitation_index(P1=P1, P2=P2, Psat=Psat), 2)
        Cv_Op = round(Kv_to_Cv(sizing['Kv']), 4)
        result['Cv_Op'] = Cv_Op
